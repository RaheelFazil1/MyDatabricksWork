...
start_time_hour=spark.sql("select case when max(timestamp) is null then to_timestamp('2021-01-01 00:00:00') else max(timestamp) end as timestamp from `databricks_poc`.`abq_litmus_agg_zorder_action_hour` where timestamp<'2339-03-21 22:20:00'")
initial_start_time = datetime.strptime("2021-01-01 00:00:00", "%Y-%m-%d %H:%M:%S")
start_time_hour=start_time_hour.withColumn("DiffInHours",round((current_timestamp().cast("long") - col("timestamp").cast("long"))/3600))
NumberofHours=start_time_hour.head()[1]
start_time = initial_start_time if window(date_trunc("second", start_time_hour.head()[0]- expr("interval 1 second")),"1 Hours").start  is None else start_time_hour.head()[0] 
end_time= datetime.now() if start_time_hour.head()[0]  is None else start_time+timedelta(hours=NumberofHours)
print(start_time)
print(end_time)
...
df=spark.sql(f'select devicetag,timestamp,avgvalue,lastvalue,firstvalue,minvalue,maxvalue,modevalue from `databricks_poc`.`abq_litmus_action_agg_zorder` where timestamp>"{start_time}" and timestamp<="{end_time}"')# 
df=df.withColumn("hour_end",window(date_trunc("second", col('timestamp')- expr("interval 1 second")),"1 hour").end )
...
# MAGIC %sql
# MAGIC merge into `databricks_poc`.`abq_litmus_agg_zorder_action_hour` target 
