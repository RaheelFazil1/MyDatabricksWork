...
start_time_hour=spark.sql("select nvl(max(timestamp),to_timestamp('2021-01-01 00:00:00')) as timestamp from `databricks_poc`.`abq_litmus_agg_zorder_hour` where timestamp<'2339-03-21 22:20:00'")
...
df=spark.sql(f'select devicetag,timestamp,avgvalue,lastvalue,firstvalue,minvalue,maxvalue,modevalue from `databricks_poc`.`abq_litmus_agg_zorder` where timestamp>"{start_time}" and timestamp<="{end_time}"') 
...
# MAGIC %sql
# MAGIC merge into `databricks_poc`.`abq_litmus_agg_zorder_hour` target 
